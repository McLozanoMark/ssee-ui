...
function updateDraft() { if (!state.draft) return; Object.assign(state.draft, { name: refs.sampleName.value.trim(), description: refs.sampleDescription.value.trim(), project: refs.sampleProject.value, source: refs.sampleSource.value, sampleSize: refs.sampleSize.value, selectionMethod: refs.selectionMethod.value, instruments: [...refs.instrumentOptions.querySelectorAll("[data-instrument]")].filter((input) => input.checked).map((input) => input.value) }); const source = getSourceConfig(state.draft.source); state.draft.population = String(source?.population || 0); state.dirty = true; updateSourceSummary(); }
...
function applyFilters() { const query = refs.filterQuery.value.trim().toLowerCase(); const source = refs.filterSource.value, status = refs.filterStatus.value; state.filteredSamples = samples.filter((sample) => { const searchable = [sample.name, sample.description, sample.source, sample.units, sample.status].join(" ").toLowerCase(); return searchable.includes(query) && (source === "Todos" || sample.source === source) && (status === "Todos" || sample.status === status); }); renderSamples(); }
